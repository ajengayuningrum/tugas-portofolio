...
if (isset($_POST['simpan'])) {
    $skill = mysqli_real_escape_string($koneksi,$_POST['skill']);
    $deskripsi_skill = mysqli_real_escape_string($koneksi,$_POST['deskripsi_skill']);
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert =  mysqli_query($koneksi, "INSERT INTO skills (skill, deskripsi_skill, foto) VALUES ('$skill', '$deskripsi_skill', '$nama_foto')");
        }
    } else {
        $insert =  mysqli_query($koneksi, "INSERT INTO skills (skill, deskripsi_skill) VALUES ('$skill', '$deskripsi_skill')");
    }
    header("location:skills.php?tambah=berhasil");
}

$id  = isset($_GET['edit']) ?  $_GET['edit'] : '';
$editUser = mysqli_query($koneksi, "SELECT * FROM skills WHERE id = '$id'");
...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $update =  mysqli_query($koneksi, "UPDATE skills SET skill='$skill', deskripsi_skill='$deskripsi_skill', foto='$nama_foto' WHERE id = '$id'");
            header("location:skills.php?ubah=berhasil");
        }
    } else {
        //ini kondisi kalo user tidak ingin memasukkan gambar
        $update =  mysqli_query($koneksi, "UPDATE skills SET skill='$skill', deskripsi_skill='$deskripsi_skill' WHERE id = '$id'");
        header("location:skills.php?ubah=berhasil");
    }
                                        ...
                                        <form action="" method="post" enctype="multipart/form-data">
                                            <div class="mb-3 row">
                                                <div class="col-sm-6">
                                                    ...
                                                    <input type="text" class="form-control" id="" name="skill" placeholder="Masukkan Skill" required
                                                        value="<?php echo isset($_GET['edit']) ? $rowEdit['skill'] : '' ?>">
                                                </div>
                                                <div class="col-sm-6">
                                                    <label for="" class="form-label">Deskripsi Skill</label>
                                                    <input type="text" class="form-control" id="" name="deskripsi_skill" placeholder="Masukkan Deskripsi" required
                                                        value="<?php echo isset($_GET['edit']) ? $rowEdit['deskripsi_skill'] : '' ?>">
                                                </div>
                                            </div>
                                            <!-- <div class=" mb-3 row">
                                                <div class="col-sm-12">
